...
> **YouTube links** need cookies or a proxy (the VM's IP is bot-checked; uploads work without either). For **cookies**: export a `cookies.txt` (how → [README → YouTube ingest](../README.md#youtube-ingest--cookies)), then copy it up like your `.env`:
> ```bash
